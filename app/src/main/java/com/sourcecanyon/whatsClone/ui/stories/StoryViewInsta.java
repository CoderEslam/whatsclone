package com.sourcecanyon.whatsClone.ui.stories;
 ...
 * @Skype : ben-_-cherif
 */
public class StoryViewInsta extends AppCompatImageView {
    private static String TAG = "StoryView";
    private static boolean DEBUG = AppConstants.DEBUGGING_MODE;
    private static final float ARC_WIDTH = 12;
    private static final int MIN_WIDTH = 300;
    private static final float CIRCLE_DIA = 0.9f;
    private static final float STROKE_WIDTH = 0.036f;
    ...
    public StoryViewInsta(Context context) {
        super(context);
        init(context, null);
    }

    public StoryViewInsta(Context context, AttributeSet attrs) {
        super(context, attrs);
        init(context, attrs);
    }

    public StoryViewInsta(Context context, AttributeSet attrs, int defStyleAttr) {
        super(context, attrs, defStyleAttr);
        init(context, attrs);
    }

    public StoryViewInsta setCircleDuration(int circleDuration) {
        this.mCircleDuration = circleDuration;
        mCircleAnim.setDuration(mCircleDuration);
        return this;
    }

    public StoryViewInsta setRotateDuration(int rotateDuration) {
    ...
    private void parseAttrs(Context context, AttributeSet attrs) {
        TypedArray typedArray = context.obtainStyledAttributes(attrs, R.styleable.StoryViewInsta);
        int startColor = typedArray.getColor(R.styleable.StoryViewInsta_start_color, mStartColor);
        int endColor = typedArray.getColor(R.styleable.StoryViewInsta_end_color, mEndColor);
        int circleDuration = typedArray.getInt(R.styleable.StoryViewInsta_circle_duration, mCircleDuration);
        int rotateDuration = typedArray.getInt(R.styleable.StoryViewInsta_rotate_duration, mRotateDuration);
        int status = typedArray.getInt(R.styleable.StoryViewInsta_status, 0);
        if (DEBUG) {
